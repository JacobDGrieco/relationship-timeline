...
export default function NodeDetailsPanel({
	...
	const typeOptions = Array.isArray(projectSettings?.nodeTypes) && projectSettings.nodeTypes.length > 0 ? projectSettings.nodeTypes : ["Default"];
	...
	const ensureRef = (fieldId) => {
		...
		return (el) => {
			inputRefs.current[fieldId].current = el;
		};
	};

	const setFieldFilter = (fieldId, value) => setFilterByField((prev) => ({ ...prev, [fieldId]: value }));
		...
		<div className={["slide-pane", isDetailsVisible && !isClosing ? "slide-in" : "", isClosing ? "slide-out" : ""].join(" ").trim()}>
			...
			<div className="details-header">
				{isEditingName ? (
						...
						onKeyDown={(e) => {
							if (e.key === "Enter") {
								...
								if (newName) {
									setGraphData((prev) => ({
										...prev,
										nodes: prev.nodes.map((n) => (n.id === selectedNode ? { ...n, label: newName, name: newName } : n)),
									}));
									try {
										cytoRef.current?.$id(String(selectedNode))?.data("label", newName);
									} catch {}
					...
					<h3 className="text-lg font-bold mb-2 editable-node-name" onClick={() => setIsEditingName(true)}>
				...
				{isEditingType ? (
					<select
						...
						onChange={(e) => {
							...
							// Mirror on Cytoscape node; stylesheet/compute can react to data('type')
							try {
								cytoRef.current?.$id(String(selectedNode))?.data("type", nextType);
							} catch {}
					...
					<div className="node-type-display" onClick={() => setIsEditingType(true)} title="Click to edit type">
						...
						{projectSettings.nodeFields.map((field, idx) => {
							...
							switch (field.type) {
									...
									return (
										<div className="details-row" key={field.id}>
											...
											<textarea
												...
												onBlur={(e) =>
													handleNodeFieldChange(selectedNode, field.id, e.target.value, setNodeDetails, {
														cytoRef,
														timelineEntries,
														setTimelineEntries,
														selectedSnapshotIndex,
														...
														{suggestions.map((opt) => (
															<div
																...
																onMouseDown={() => {
																	...
																	setFieldFilter(field.id, "");
																	inputRefs.current[field.id]?.current &&
																		(inputRefs.current[field.id].current.value = "");
								...
								case "dynamic-multiselect": {
									...
									const fromNodes = Array.from(new Set(Object.values(nodeDetails).flatMap((nd) => nd?.[field.id] || [])));
									const suggestions = Array.from(new Set([...fromOptions, ...fromNodes])).filter(
										(opt) => opt.toLowerCase().includes(filter) && !selected.includes(opt)
									);
			...
			<button
				...
				onClick={() => {
					handleDeleteNode(cytoRef, selectedNode, setGraphData, setNodeDetails);
					setIsDetailsVisible(false);
					setJustClosedRecently(false);

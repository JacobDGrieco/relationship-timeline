					...
					{activePanel === PANEL.MENU && (
						<>
							<div className="settings-cards">
								{cards.map((card) => (
									<button key={card.key} className="settings-card" onClick={() => setActivePanel(card.key)}>
				...
				{activePanel !== PANEL.MENU && (
					...
					>
						{activePanel === PANEL.NAME && (
							<ProjectName projectName={projectName} setProjectName={setProjectName} onClose={closeSidePanel} />
						)}
						...
						{activePanel === PANEL.STYLES && (
							<NodeConnectionCSSPopup
								...
								setProjectSettings={setProjectSettings}
								availableFieldNames={
									Array.isArray(projectSettings.nodeFields)
						...
						{activePanel === PANEL.CHANGELOG && (
							<div style={{ display: "grid", gap: "1rem" }}>
								<p style={{ opacity: 0.8, marginTop: 0 }}>
									Planned: automatically record snapshots and edits; view, filter, and restore from the log.
								</p>
								<div className="actions">
									<button className="cancel" onClick={closeSidePanel}>
										Back
									</button>
								</div>
							</div>
						...
						{activePanel === PANEL.STATS && (
							<div style={{ display: "grid", gap: "1rem" }}>
								<p style={{ opacity: 0.8, marginTop: 0 }}>
									Planned: calculate different statistics such as "popularity", distances, etc.
								</p>
								<div className="actions">
									<button className="cancel" onClick={closeSidePanel}>
										Back
